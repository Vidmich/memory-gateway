...
gateway screens.
**Size:** L
**Status:** post-v1. 100 supplies the free labels; 104 is what makes "before vs after a
configuration change" a comparison between two known states.
...
- [ ] `ChunkAudit.run(connector_id)` scrolls the live collection once (the vector port's
      ...
      age shown.
- [ ] Findings, each with a count and the documents behind it: chunks under a floor (default
      ...
      report names it).
- [ ] Each finding links to Compare with the offending document preselected, because the page
      that finds a badly cut file should open the page that recuts it.
- [ ] The report is **per format** as well as overall, keyed by task 20's `format_label`,
...
- [ ] `EmbeddingAudit.run(connector_id)`: dimension of stored vectors against the platform
...
- [ ] **Drift against a fresh sample**: re-embed a bounded sample (default 100 chunks, capped
...
- [ ] `embedding_model` recorded on the collection (SPEC §9.4) is compared with what every
      document row says it was embedded with; disagreement is a finding, not a fix.
- [ ] Findings link to the reindex action for the connector, or to the platform reindex when
      the model itself is the problem.

### Evaluation sets (per gateway)

- [ ] Tables: `evaluation_sets(id, organization_id, gateway_id, name, created_by, ...)` and
...
- [ ] Labels can be **chunk-level or document-level**. A person can usually say *which
...
- [ ] **Import from the log**: pick a window and a filter, get one item per distinct user
      ...
      separately.
- [ ] **Generate** (optional, spends): for a sample of chunks, ask a model to write the
...
- [ ] Negative items: a question with **no** relevant chunk, so that precision has something to
      be wrong about and `doc_min_score` has something to defend.
- [ ] Items survive reindexing. A `relevant_chunk_id` is invalidated by a recut (task 104
...
- [ ] `EvaluationRun.run(set_id, config_patch=None)` calls the same `MemoryService` the data
...
- [ ] Per item: chunks retrieved with scores, which were relevant, rank of the first relevant.
...
- [ ] Runs are stored with the effective gateway configuration, the connector chunk
...
- [ ] A run costs embedding calls (one per question); it is a background job with progress
      and a cap on items per run. A set of 5 000 questions is a batch, not a click.

### UI

- [ ] **Connectors → Validation** with two tabs, Chunking and Embeddings: the report, its age,
...
- [ ] **Gateways → Validation**: evaluation sets (list, create, import, generate, edit items
...
- [ ] Try retrieval gains **Add to evaluation set** — the moment a person tuning a gateway sees
      the right chunk come back is the moment the label is cheapest.
- [ ] The Dashboard's degraded list includes a connector whose last chunking or embedding audit
      raised a red finding.

### Spec

- [ ] Add §6.6 *Retrieval evaluation* (sets, runs, what the numbers mean and what they cannot
      mean); amend §13.1 with the three screens; amend §10.1 with the audit findings as a
      health signal.

## Acceptance criteria

- [ ] The chunking report over a fixture connector matches `distribution()` computed over the
      same points, and each finding's count matches a hand count on the fixture.
- [ ] The embedding audit flags a collection where 10% of vectors are identical, one where the
...
- [ ] Intra-document agreement on a fixture where every document is about a different subject
...
- [ ] An evaluation run's retrieved chunks for each question are identical to Try retrieval's
      for the same question and configuration — asserted, not assumed.
- [ ] recall@k, precision@k and MRR on a hand-computed fixture match to the decimal, at both
...
- [ ] Generated items are marked, and a run including them says so in its headline.
